...
以下为目标表；测试数据可清空，因此以新 schema/替换旧 `prepaid_wallets`、`prepaid_recharges` 与旧 `billing_reservations` 为准，不做兼容迁移。
...
结论：在“同一套餐有效期内不能重复购买、只允许升级”“即买即用但到期顺延”“无 Tenant 共享池”“两类金额余额”“外部平台 Company 不直接向我方付费”“Personal 不可转企业”的前提下，本设计不存在尚未闭合的业务链路。实施中不得恢复旧 `prepaid_wallets` 的 Company 单余额模型，也不得在任何不足路径隐式回退到 Tenant。
