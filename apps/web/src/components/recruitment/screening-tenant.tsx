...
function ResultList({ result }: { result: ScreeningRun }) { return <div className="mt-4 space-y-3"><p className="text-xs text-[#7185a3]">{result.status === "RUNNING" ? `AI 正在筛选，当前进度 ${result.progress}%` : "筛选已结束，可在企业账单中查看本账号的积分消耗。"}</p>{result.items.map(item => <article key={item.id} className="rounded-xl border border-[#dde8f2] p-4"><div className="flex justify-between gap-3"><div className="flex gap-3"><strong className="text-sm text-[#26466f]">{item.candidateName}</strong><ItemStatus status={item.status} errorCode={item.errorCode}/></div>{item.score !== null && <strong className="text-2xl text-[#1676d2]">{item.score}<small className="text-xs font-normal">/100</small></strong>}</div>{item.status === "SUCCEEDED" && <div className="mt-3 grid gap-3 md:grid-cols-2"><ResultGroup label="匹配点" values={item.matchedPoints}/><ResultGroup label="不符点" values={item.unmatchedPoints}/><ResultGroup label="可协商项" values={item.negotiablePoints}/><ResultGroup label="缺失信息" values={item.missingInformation}/><ResultGroup label="风险与提示" values={item.risks}/><ResultGroup label="证据" values={item.evidence}/></div>}</article>)}</div>; }
