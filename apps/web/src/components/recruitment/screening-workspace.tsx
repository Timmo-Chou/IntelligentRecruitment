...
export function ScreeningWorkspace({ embedded = false, recruitmentTaskId, initialJobId }: { embedded?: boolean; recruitmentTaskId?: string; initialJobId?: string | null }) {
  ...
  async function start() {
    ...
    try {
      ...
      const run = await startScreeningRun(workspaceId, plan.id, ids, quote.id, crypto.randomUUID());
  ...
  const body = <>{error && <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[#fecaca] bg-[#fff1f2] px-4 py-3 text-sm text-[#b42318]"><AlertCircle size={17}/>{error}{error.includes("充值") && <Link href="/billing/recharge" className="ml-auto rounded-md border border-[#f4a2a2] bg-white px-3 py-1 text-xs font-semibold text-[#b42318]">去充值</Link>}</div>}
    {loading ? <Loading text="正在读取筛选数据..."/> : <section className="mt-4 rounded-xl border border-[#d6e5f5] bg-white p-5 shadow-[0_6px_20px_rgba(30,92,160,0.04)]">
      ...
      {resultMode ? <ResultList result={result!}/> : <div className="mt-5 space-y-6">
        ...
        <section><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="section-title">候选人</h3><p className="mb-0 mt-1 text-xs text-[#7185a3]">默认选择人才库全部已解析候选人，可在弹窗中筛选范围。</p></div><button type="button" className="outline-button" onClick={() => setPickerOpen(true)}><UsersRound size={14}/>人才库筛选（已选 {selected.size} 人）</button></div><div className="mt-4 flex justify-between rounded-lg bg-[#f6f9fd] px-4 py-3 text-sm"><span>本次将筛选 {selected.size} 份简历</span><strong className="text-[#0c8c69]">预计 ¥{money(selected.size * (pricing?.unitPriceMinor || 0))}</strong></div><button type="button" className="primary-button mt-4 w-full justify-center" disabled={busy || !jobId || !selected.size || totalWeight !== 100} onClick={() => void start()}>{busy ? <Loader2 className="animate-spin" size={15}/> : <Filter size={15}/>}确认费用并开始筛选</button></section>
      </div>}</section>}
...
// BOSS monetary values use micro-CNY (1 CNY = 1,000,000).
function money(value: number) { return (value / 1_000_000).toFixed(2); }
