...
export function ScreeningWorkspace({ embedded = false, recruitmentTaskId, initialJobId }: { embedded?: boolean; recruitmentTaskId?: string; initialJobId?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setJobId(current => current || (initialJobId && jobResult.items.some(job => job.id === initialJobId)
        ? initialJobId : jobResult.items[0]?.id || ""));
  ...
  async function handleCreatePlan() {
    ...
    try {
      ...
      const created = await createScreeningPlan(workspaceId, { jobId, name: `${job?.title || "职位"}筛选方案`, dimensions, recruitmentTaskId });
      setPlans(await fetchScreeningPlans(workspaceId, recruitmentTaskId)); setPlanId(created.id); setDimensions(created.dimensions);
    } catch (cause) { setError(messageOf(cause)); }
  ...
  async function handleSavePlan() {
    ...
    setBusy(true); setError(null);
    try { const next = await updateScreeningPlan(workspaceId, planId, dimensions); setDimensions(next.dimensions); setPlans(await fetchScreeningPlans(workspaceId, recruitmentTaskId)); }
    catch (cause) { setError(messageOf(cause)); }
    finally { setBusy(false); }
  }
    ...
    for (let attempt = 0; attempt < 240; attempt += 1) {
      ...
      try {
        ...
        if (next.status !== "RUNNING") {
          setRuns(await fetchScreeningRuns(targetWorkspaceId, recruitmentTaskId));
          return;
        }
      } catch (cause) {
  ...
  async function handleRun() {
    ...
    try {
      ...
      setResult(next); setRuns(await fetchScreeningRuns(workspaceId, recruitmentTaskId)); void pollRun(workspaceId, next.id);
    } catch (cause) { setError(messageOf(cause)); }
  ...
  async function handleCancel() {
    ...
    setBusy(true); setError(null);
    try { const next = await cancelScreeningRun(workspaceId, result.id, crypto.randomUUID()); setResult(next); setRuns(await fetchScreeningRuns(workspaceId, recruitmentTaskId)); }
    catch (cause) { setError(messageOf(cause)); }
    finally { setBusy(false); }
  }
  ...
  return embedded ? content : <AppShell activeItem="简历筛选" pageHeader={
    <section className="flex flex-wrap items-end justify-between gap-3">
      <div><h1 className="m-0 text-[25px] font-bold text-[#09245d]">简历筛选</h1><p className="mb-0 mt-1 text-sm text-[#60799f]">{workspace?.name ?? "当前工作空间"} · 固定职位版本、解析版本和筛选规则后执行可解释匹配</p></div>
      <div className="flex items-center gap-2 rounded-lg border border-[#cfe4f5] bg-white px-3 py-2 text-xs text-[#53709a]"><CircleDollarSign size={16} className="text-[#0a9a66]"/>{pricing ? `临时价 ¥${money(pricing.unitPriceMinor)}/成功候选人` : "正在读取计价规则"}</div>
    </section>
  }>{content}</AppShell>;
}
