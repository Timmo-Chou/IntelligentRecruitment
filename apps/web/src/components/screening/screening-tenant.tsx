...
import { AlertCircle, CheckCircle2, Filter, Loader2, Pencil, RefreshCw, Save, Square, Target, UsersRound, X } from "lucide-react";
...
export function ScreeningTenant({ embedded = false, recruitmentTaskId, initialJobId }: { embedded?: boolean; recruitmentTaskId?: string; initialJobId?: string | null }) {
  ...
  const body = <>{error && <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[#fecaca] bg-[#fff1f2] px-4 py-3 text-sm text-[#b42318]"><AlertCircle size={17}/>{error}</div>}
    {loading ? <Loading text="正在读取筛选数据..."/> : <section className="mt-4 rounded-xl border border-[#d6e5f5] bg-white p-5 shadow-[0_6px_20px_rgba(30,92,160,0.04)]">
      ...
      {resultMode ? <ResultList result={result!}/> : <div className="mt-5 space-y-6">
        ...
        <section><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="section-title">候选人</h3><p className="mb-0 mt-1 text-xs text-[#7185a3]">默认选择人才库全部已解析候选人，可在弹窗中筛选范围。</p></div><button type="button" className="outline-button" onClick={() => setPickerOpen(true)}><UsersRound size={14}/>人才库筛选（已选 {selected.size} 人）</button></div><div className="mt-4 rounded-lg bg-[#f6f9fd] px-4 py-3 text-sm">本次将筛选 {selected.size} 份简历，所需积分将由企业共享积分池按实际 AI 调用扣除。</div><button type="button" className="primary-button mt-4 w-full justify-center" disabled={busy || !jobId || !selected.size || totalWeight !== 100} onClick={() => void start()}>{busy ? <Loader2 className="animate-spin" size={15}/> : <Filter size={15}/>}开始筛选</button></section>
      </div>}</section>}
  ...
  return embedded ? body : <AppShell activeItem="简历筛选" pageHeader={<section><h1 className="m-0 text-[25px] font-bold text-[#09245d]">简历筛选</h1><p className="mb-0 mt-1 text-sm text-[#60799f]">批量调用 AI 评估候选人与职位的匹配程度</p></section>}>{body}</AppShell>;
}

function ResultList({ result }: { result: ScreeningRun }) { return <div className="mt-4 space-y-3"><p className="text-xs text-[#7185a3]">{result.status === "RUNNING" ? `AI 正在筛选，当前进度 ${result.progress}%` : result.settlementStatus === "PENDING" ? "筛选已完成，积分结算处理中。" : result.settlementStatus === "SETTLEMENT_FAILED" ? "筛选已完成，但积分结算失败，请联系平台运营。" : "筛选已结束，可在企业账单中查看本账号的积分消耗。"}</p>{result.items.map(item => <article key={item.id} className="rounded-xl border border-[#dde8f2] p-4"><div className="flex justify-between gap-3"><div className="flex gap-3"><strong className="text-sm text-[#26466f]">{item.candidateName}</strong><ItemStatus status={item.status} errorCode={item.errorCode}/></div>{item.score !== null && <strong className="text-2xl text-[#1676d2]">{item.score}<small className="text-xs font-normal">/100</small></strong>}</div>{item.status === "SUCCEEDED" && <div className="mt-3 grid gap-3 md:grid-cols-2"><ResultGroup label="匹配点" values={item.matchedPoints}/><ResultGroup label="不符点" values={item.unmatchedPoints}/><ResultGroup label="可协商项" values={item.negotiablePoints}/><ResultGroup label="缺失信息" values={item.missingInformation}/><ResultGroup label="风险与提示" values={item.risks}/><ResultGroup label="证据" values={item.evidence}/></div>}</article>)}</div>; }
