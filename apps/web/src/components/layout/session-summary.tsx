...
export function SessionSummary() {
  const [me,setMe]=useState<Me|null>(null); const [balance,setBalance]=useState<number|null>(null);
  const [companyName,setCompanyName]=useState<string|null>(null);
  useEffect(()=>{ let active=true; (async()=>{ try { const [user,spaces,companies]=await Promise.all([apiFetch<Me>("/me"),apiFetch<Workspace[]>("/workspaces"),apiFetch<Company[]>("/companies")]); if(!active)return; setMe(user); setCompanyName(companies[0]?.displayName??null); const selected=spaces.find(item=>item.hasDataAccess)??null; if(selected){const bill=await apiFetch<{availableAmountMinor:number}>(`/workspaces/${selected.id}/billing`);if(active)setBalance(bill.availableAmountMinor);} } catch (error) { if(active && error instanceof ApiError && error.status===401) window.location.replace("/login"); } })(); return()=>{active=false};},[]);
  ...
  return <div className="flex items-center gap-3 text-xs text-[#405781]">
    ...
    <div className="hidden items-center gap-2 border-l border-[#d9e6f3] pl-3 md:flex"><span className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-[#dff7f4] to-[#b7d5ff] text-[#0d57aa]"><UserRound size={18}/></span><span className="leading-tight"><strong className="block text-[#10285b]">{companyName??me?.displayName??"招聘用户"}</strong><small>{me?.maskedPhone??"未登录"}</small></span></div>
