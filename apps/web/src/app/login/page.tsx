...
type AuthResponse = { access_token: string; new_user: boolean; password_setup_required: boolean };

/** BOSS owns registration and password credentials; Recruitment only presents the existing login experience. */
export default function LoginPage() {
  ...
  async function continueAfterAuthentication(needsOnboarding = false) {
    // WorkspaceProvider can have observed a 401 before authentication completed.
    // Reload BOSS-backed contexts before opening a protected business page.
    await refreshWorkspaces();
    router.replace(needsOnboarding ? "/onboarding" : "/recruitment");
  }
  ...
  async function sendCode() {
    ...
    try {
      const result = await apiFetch<{ challenge_id: string; mock_code?: string }>("/auth/challenges", {
        method: "POST", body: JSON.stringify({ phone, purpose: resetMode ? "PASSWORD_RESET" : "LOGIN" }),
      }, false);
      setChallengeId(result.challenge_id); setSeconds(60);
  ...
  async function submit(event: FormEvent) {
    ...
    if ((mode === "code" || resetMode) && !challengeId) { setMessage("请先获取验证码"); return; }
    setLoading(true); setMessage(null);
    try {
      const result = resetMode
        ? await apiFetch<AuthResponse>("/auth/password-reset", {
            method: "POST", body: JSON.stringify({ challenge_id: challengeId, phone, code, new_password: password }),
          }, false)
        : mode === "code"
      ...
      if (result.password_setup_required) {
        setRequiresOnboarding(result.new_user);
        setShowPasswordSetup(true);
      } else await continueAfterAuthentication(result.new_user);
  ...
  function startReset() {
    setMode("password"); setResetMode(true); setCode(""); setChallengeId(null); setMessage("请输入验证码和新密码");
  }

  return <main className="login-canvas min-h-screen p-5 text-[#10285b] lg:p-10"><div className="mx-auto grid min-h-[calc(100vh-40px)] max-w-[1500px] gap-6 lg:grid-cols-[460px_minmax(0,1fr)]">
    <section className="flex flex-col rounded-[26px] border border-white/80 bg-white/90 p-7 shadow-[0_18px_60px_rgba(39,100,180,0.09)] sm:p-10">
      <div><span className="flex items-center gap-3 text-lg font-bold text-[#09245d]"><span className="brand-mark" aria-hidden="true"><i/><i/></span>AI招聘工作台</span><h1 className="mb-0 mt-10 text-[34px] font-bold tracking-tight text-[#071b4b]">欢迎登录</h1><p className="mt-3 text-sm text-[#607697]">使用 BOSS 统一账号，首次手机号验证后自动注册</p></div>
      ...
      <form className="mt-8 space-y-5" onSubmit={submit}>
        ...
        {mode === "code" || resetMode ? <label className="block"><span className="mb-2 block text-sm font-medium">验证码</span><span className="login-input"><LockKeyhole size={18}/><input value={code} onChange={event => setCode(event.target.value)} type="text" inputMode="numeric" autoComplete="one-time-code" placeholder="请输入验证码" aria-label="验证码" maxLength={6}/><button type="button" onClick={sendCode} disabled={loading || phone.length !== 11 || seconds > 0} className="whitespace-nowrap border-l border-[#dbe5f2] pl-4 text-sm font-semibold text-[#176ce5] disabled:text-[#9aabc0]">{seconds > 0 ? `${seconds}秒后重试` : "获取验证码"}</button></span></label> : <label className="block"><span className="mb-2 block text-sm font-medium">密码</span><span className="login-input"><KeyRound size={18}/><input value={password} onChange={event => setPassword(event.target.value)} type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder="请输入登录密码" aria-label="密码" maxLength={64}/><button type="button" onClick={() => setShowPassword(value => !value)} className="text-[#6d82a2]" aria-label={showPassword ? "隐藏密码" : "显示密码"}>{showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button></span></label>}
        {resetMode && <label className="block"><span className="mb-2 block text-sm font-medium">新密码</span><span className="login-input"><KeyRound size={18}/><input value={password} onChange={event => setPassword(event.target.value)} type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder="8至64位，包含字母和数字" aria-label="新密码" maxLength={64}/><button type="button" onClick={() => setShowPassword(value => !value)} className="text-[#6d82a2]" aria-label={showPassword ? "隐藏密码" : "显示密码"}>{showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button></span></label>}
        ...
        <label className="flex items-start gap-2 text-xs leading-5 text-[#60799f]"><input checked={agreed} onChange={event => setAgreed(event.target.checked)} className="mt-0.5" type="checkbox"/><span>我已阅读并同意<Link href="/terms" className="text-[#176ce5] hover:underline" target="_blank">《用户服务协议》</Link>和<Link href="/privacy" className="text-[#176ce5] hover:underline" target="_blank">《隐私政策》</Link></span></label>
        <button className="login-submit disabled:opacity-60" disabled={loading || !agreed || phone.length !== 11 || ((mode === "code" || resetMode) && code.length !== 6) || (mode === "password" && password.length < 8)} type="submit">{loading ? "处理中…" : resetMode ? "重置密码并登录" : mode === "code" ? "登录 / 注册" : "登录"}</button>
      </form>
    </section>
  ...
  </div>{showPasswordSetup && <PasswordSetupDialog onComplete={() => continueAfterAuthentication(requiresOnboarding)}/>}</main>;
}

function PasswordSetupDialog({ onComplete }: { onComplete: () => Promise<void> }) {
  const [password, setPassword] = useState("");
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#071b4b]/45 p-5 backdrop-blur-sm" role="presentation">
    <section className="w-full max-w-md rounded-[24px] border border-white bg-white p-7 shadow-[0_24px_80px_rgba(7,27,75,.25)]" role="dialog" aria-modal="true" aria-labelledby="password-setup-title">
      ...
      <p className="mt-2 text-sm leading-6 text-[#60799f]">首次验证码登录需要设置密码。完成后请选择个人使用或企业注册。</p>
