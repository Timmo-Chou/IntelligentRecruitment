...
export default function OverviewPage() {
  ...
  useEffect(()=>{if(!tenantId)return;
    Promise.all([
      ...
      apiFetch<JobStats>(`/tenants/${tenantId}/jobs/stats`).catch(()=>null)
  ...
  return <AppShell activeItem="概览" pageHeader={
    ...
    <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="招聘概览">
      ...
      <Link href="/candidates" className="metric-card !min-h-[112px] transition hover:-translate-y-0.5 hover:border-[#b9d9ed]">
        ...
        <div className="min-w-0">
          ...
          <p className="mb-0 mt-2 truncate text-xs text-[#6c83a7]">当前 Tenant 人才库</p>
        </div>
      </Link>
      {/* 进行中任务 */}
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="rounded-xl border border-[#d6e5f5] bg-white p-5 shadow-[0_6px_20px_rgba(30,92,160,0.04)]">
        <div className="flex items-center justify-between"><div><h2 className="m-0 text-base font-bold text-[#173568]">招聘任务</h2><p className="mb-0 mt-1 text-xs text-[#7187a8]">当前 Tenant 的真实任务状态</p></div><Link href="/recruitment" className="flex items-center gap-1 text-xs font-semibold text-[#1672df]">查看全部 <ArrowRight size={14}/></Link></div>
