...
export default function BillingPage(){const[spaces,setSpaces]=useState<Workspace[]>([]);const[selected,setSelected]=useState("");const[data,setData]=useState<Billing|null>(null);const[error,setError]=useState("");
  ...
  return <AppShell activeItem="额度与账本"><section className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="m-0 text-[25px] font-bold text-[#09245d]">额度与账本</h1><p className="mt-1 text-sm text-[#55709d]">试用金按 Workspace 归属，所有资金变动保留不可变流水。</p></div><select value={selected} onChange={e=>setSelected(e.target.value)} className="h-10 rounded-lg border border-[#bdd3ef] bg-white px-3 text-sm">{spaces.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></section>
