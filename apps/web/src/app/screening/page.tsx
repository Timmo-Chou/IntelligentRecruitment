...
import { cancelScreeningRun, createScreeningPlan, createScreeningQuote, defaultScreeningDimensions, fetchScreeningPlans, fetchScreeningRun, fetchScreeningRuns, retryFailedScreening, startScreeningRun, updateScreeningPlan, type ScreeningDimension, type ScreeningPlan, type ScreeningRun, type ScreeningRunSummary } from "@/lib/screening-api";
...
export default function ScreeningPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [jobResult, candidateResult, planResult, runResult] = await Promise.all([
        fetchJobs(workspaceId, { page: 1, pageSize: 100 }), fetchCandidates(workspaceId, "", "PARSED"),
        fetchScreeningPlans(workspaceId), fetchScreeningRuns(workspaceId),
      ]);
      setJobs(jobResult.items); setCandidates(candidateResult.items); setPlans(planResult); setRuns(runResult);
      setJobId(current => current || jobResult.items[0]?.id || "");
  ...
  async function handleRun() {
    if (!workspaceId || !planId || selectedCandidates.size === 0) return;
    setBusy(true); setError(null);
    try {
  ...
  async function handleRetry() {
    ...
    if (!window.confirm(`将重试 ${failedCount} 个失败项；仍按 ¥0.80/成功候选人结算。是否继续？`)) return;
    setBusy(true);
    try { const next = await retryFailedScreening(workspaceId, result.id); setResult(next); setRuns(await fetchScreeningRuns(workspaceId)); }
    catch (cause) { setError(messageOf(cause)); }
    finally { setBusy(false); }
  }
  ...
  async function handleCancel() {
    ...
    setBusy(true); setError(null);
    try { const next = await cancelScreeningRun(workspaceId, result.id); setResult(next); setRuns(await fetchScreeningRuns(workspaceId)); }
    catch (cause) { setError(messageOf(cause)); }
    finally { setBusy(false); }
  }
  ...
  return <AppShell activeItem="简历筛选">
    <section className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="m-0 text-[25px] font-bold text-[#09245d]">简历筛选</h1><p className="mb-0 mt-1 text-sm text-[#60799f]">{workspace?.name} · 固定职位版本、解析版本和筛选规则后执行可解释匹配</p></div><div className="flex items-center gap-2 rounded-lg border border-[#cfe4f5] bg-white px-3 py-2 text-xs text-[#53709a]"><CircleDollarSign size={16} className="text-[#0a9a66]"/>临时价 ¥0.80/成功候选人</div></section>
    ...
    {loading ? <div className="grid h-64 place-items-center"><Loader2 className="animate-spin text-[#1784d8]"/></div> : <div className="mt-4 grid gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
      <aside className="space-y-4">
        ...
        <Card title="2. 选择候选人" icon={<UsersRound/>}>
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-[#60799f]"><span>已选 {selectedCandidates.size} 人</span><strong className="text-[#0c8c69]">预计 ¥{(selectedCandidates.size * 0.8).toFixed(2)}</strong></div>
          <select value={scenario} onChange={(event) => setScenario(event.target.value)} className="field-control mt-3"><option value="NORMAL">正常筛选</option><option value="PARTIAL_FAILURE">模拟部分失败</option><option value="INVALID_SCHEMA">模拟非法结果</option></select>
      ...
      <main className="min-w-0 space-y-4">
        <section className="rounded-xl border border-[#d6e5f5] bg-white p-5 shadow-[0_6px_20px_rgba(30,92,160,0.04)]">
          <div className="flex items-center justify-between"><div><h2 className="m-0 text-base text-[#173568]">筛选结果</h2>{result && <p className="mb-0 mt-1 text-xs text-[#7185a3]">{result.jobTitle} · {result.planName} · 实际结算 ¥{(result.settledAmountMinor / 100).toFixed(2)}</p>}</div><div className="flex gap-2">{result?.status === "RUNNING" && <button type="button" className="outline-button" disabled={busy} onClick={() => void handleCancel()}><Square size={13}/>取消任务</button>}{result?.items.some(item => item.status === "FAILED") && <button type="button" className="outline-button" disabled={busy} onClick={() => void handleRetry()}><RefreshCw size={14}/>重试失败项</button>}</div></div>
          {!result ? <div className="flex h-[420px] flex-col items-center justify-center text-center"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#eaf8f5] text-[#169b83]"><Target size={27}/></span><h3 className="mb-0 mt-4 text-lg text-[#193866]">选择方案和候选人开始筛选</h3><p className="mt-2 text-sm text-[#7185a3]">结果会展示匹配点、不符点、可协商项、缺失信息、风险和证据。</p></div> : <div className="mt-4 space-y-3">{result.items.map(item => <article key={item.id} className="rounded-xl border border-[#dde8f2] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><strong className="text-sm text-[#26466f]">{item.candidateName}</strong>{item.status === "SUCCEEDED" ? <span className="flex items-center gap-1 text-xs text-[#108760]"><CheckCircle2 size={14}/>完成</span> : <span className="text-xs text-[#c24b3f]">失败 · {item.errorCode}</span>}</div>{item.score !== null && <strong className={`text-2xl ${item.score >= 85 ? "text-[#07945f]" : item.score >= 70 ? "text-[#1676d2]" : "text-[#d17a20]"}`}>{item.score}<small className="text-xs font-normal">/100</small></strong>}</div>{item.status === "SUCCEEDED" && <><div className="mt-3 grid gap-3 md:grid-cols-2"><ResultGroup label="匹配点" values={item.matchedPoints} tone="good"/><ResultGroup label="不符点" values={item.unmatchedPoints} tone="bad"/><ResultGroup label="可协商项" values={item.negotiablePoints}/><ResultGroup label="缺失信息" values={item.missingInformation}/><ResultGroup label="风险与提示" values={item.risks}/><ResultGroup label="证据" values={item.evidence}/></div><p className="mb-0 mt-3 text-[10px] text-[#8a6e37]">AI 结果仅供辅助，不能自动淘汰、拒绝或替代招聘人员最终判断。</p></>}</article>)}</div>}
        </section>
